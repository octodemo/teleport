...
Update`ssl_cert`, `ssl_key`, and `ssl_certificate_authorities` to the include
the locations of the server certificate, server key, and certificate authority
files you generated earlier by running the `teleport-event-handler configure`
command. Logstash will authenticate client certificates against the CA file and
present a signed certificate to the Teleport event-handler plugin.
...
Make sure the pipeline started successfully by running the following command to
tail Logstash's logs:

```code
$ sudo journalctl -u logstash -f
```

{/* TODO: Indicate how to tell the pipeline started successfully*/}
