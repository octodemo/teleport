...
Teleport comes with several pre-configured roles:
...
The default `access` role we assigned to a user in this guide is probably too
...
integrating Teleport with SSO providers.
...
plugins](../access-controls/access-request-plugins.mdx) so users can request and
review Access Requests using your existing communication workflows.
